...
非ランニング系のワークアウト（ウォーキング、筋トレ等）は取り込み対象外（フィルタで除外）。
...
```
- ワークアウト種別を判定する専用フィールド（`activityType`等）が存在しないため、
  ローカライズされた `name` 文字列に `"ランニング"` または `"run"`（大文字小文字無視）が
  含まれるかで判定している（`RUNNING_NAME_MARKERS`）
